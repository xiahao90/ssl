...
openssl genrsa -aes256 -passout pass:${password} -out private/rootca.key.pem 4096
# 生成自签名的CA根证书
openssl req -config openssl.cnf -new -x509 -days 36500 -sha256 -extensions v3_ca -key private/rootca.key.pem -out certs/rootca.cert.pem -passin file:pass.txt -subj "/C=CN/ST=Shanghai/L=Shanghai/O=Internet Widgits Pty Ltd/OU=Certificate Authority/CN=Pty Root CA"
# 查看证书内容, 以确保证书生成正确
...
# 用CA根证书签发该代理CA证书请求
openssl x509 -req -extfile openssl.cnf -extensions v3_ca -days 36500 -sha256 -CA certs/rootca.cert.pem -CAkey private/rootca.key.pem -CAcreateserial -CAserial serial -in csr/intermediateca.csr.pem -out certs/intermediateca.cert.pem -passin file:pass.txt
...
# 用代理CA证书签发证书
openssl x509 -req -extfile openssl.cnf -extensions usr_cert -days 36500 -sha256 -CA certs/intermediateca.cert.pem -CAkey private/intermediateca.key.pem -CAserial serial -in newcerts/${domain}/server.csr.pem -out newcerts/${domain}/server.cert.pem -passin file:pass.txt
# 查看生成的证书内容
...
cat newcerts/${domain}/server.cert.pem certs/intermediateca.cert.pem certs/rootca.cert.pem > newcerts/${domain}/server-chain.cert.pem
...
openssl x509 -req -extfile openssl.cnf -extensions usr_cert -days 36500 -sha256 -CA certs/intermediateca.cert.pem -CAkey private/intermediateca.key.pem -CAserial serial -in newcerts/${domain}/client.csr.pem -out newcerts/${domain}/client.cert.pem -passin file:pass.txt
...
openssl pkcs12 -export -clcerts -in newcerts/${domain}/client.cert.pem -inkey newcerts/${domain}/client.key.pem -out newcerts/${domain}/client.p12 -passout pass:${password}
